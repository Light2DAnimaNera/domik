...
бот спрашивает, есть ли вам 18 лет. При ответе «Да» выводится системное
сообщение с правилами использования и устанавливается меню команд согласно роли
пользователя. Если выбрать «Нет», учётная запись блокируется и дальнейший доступ
к боту запрещён.

Баланс хранится в колонке `credits` таблицы `users`. Стоимость списания рассчитывается по числу токенов: один токен запроса стоит `0.0020` единиц, токен ответа — `0.0005`. Итоговая сумма умножается на коэффициент `token_cost_coeff` из таблицы `settings` (по умолчанию он равен `1.0`). Команда `/balance` показывает текущий баланс и сегодняшние траты, оба значения округляются вверх до двух знаков после запятой и сопровождаются символом 🝣. Команда `/coeff` позволяет узнать тарифный коэффициент, а `/set_coeff` — изменить его. Команды `/coeff` и `/set_coeff` доступны только администратору.
Новые пользователи при регистрации получают 100 кредитов на счёт.
В таблице `users` присутствует поле `blocked`. Если его значение равно `1`,
...
Сессия автоматически завершается, если пользователь не отправляет сообщения более 60 секунд.
