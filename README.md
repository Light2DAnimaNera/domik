...
пользователя в личном чате создаётся топик в супергруппе `DSS_FORUM_ID`, куда
пересылается паспорт клиента. Связка `user_id` ↔ `topic_id` сохраняется в базе
данных и загружается в кеш при старте, поэтому все дальнейшие сообщения
попадают в тот же топик. База данных содержит уникальный индекс по `topic_id`,
что гарантирует использование одного топика для каждого пользователя.
