...
Бот `DSS` соединяет клиентов с операторами в форуме. При первом сообщении
пользователя в личном чате бот ищет связку `user_id` ↔ `topic_id` в отдельной
базе `dss_topics.db`. Если запись не найдена, создаётся топик в супергруппе
`DSS_FORUM_ID` и туда отправляется паспорт клиента. Связка сохраняется в этой
отдельной базе и загружается в кеш при старте, поэтому все дальнейшие сообщения
попадают в тот же топик. Таблица имеет уникальный индекс по `topic_id`.
Ответы операторов в этом топике пересылаются пользователю без подписи.
Если оператор отвечает на пересланное сообщение, бот определяет ID клиента из
поля `forward_from`. При ответе на паспорт с текстом `ID: <число>` ID берётся из
этого текста. Полученный идентификатор сохраняется, и ответ приходит как реплай
к исходному посту пользователя.
