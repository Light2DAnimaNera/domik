...
Записи со статусом `scheduled` проверяются фоновым планировщиком раз в минуту.
Если время отправки меньше текущего, сообщение рассылается выбранной
аудитории, поле `sent_at` заполняется текущим моментом и статус меняется на
`sent`.
