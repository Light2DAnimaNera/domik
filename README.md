...
В консоль выводятся логи со статусом оплаты от создания счёта до его завершения. Дополнительно сведения сохраняются в таблицу `payments`: пользователь, сумма, время в формате `mm-dd-yy HH-MM` и текущий статус платежа.
