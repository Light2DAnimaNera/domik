...
базе `dss_topics.db`. Если записи нет, создаётся топик в супергруппе
`DSS_FORUM_ID` и туда отправляется паспорт клиента. В таблице также хранится
`passport_message_id` — ID отправленного паспорта. Связка сохраняется в этой
...
Если сообщение отправлено в ответ на обращение клиента, оно приходит как
реплай к исходному посту пользователя.
