...
- В консоль выводятся логи, отражающие статус оплаты от создания счёта до завершения. Дополнительно данные записываются в таблицу `payments`, где для каждого платёжного идентификатора хранится одна запись с текущим статусом и временем `mm-dd-yy HH-MM`.
