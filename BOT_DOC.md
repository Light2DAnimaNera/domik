...
- `/start` — спрашивает, есть ли пользователю 18 лет. При ответе «Да» выводится системное сообщение с правилами, иначе доступ блокируется.
...
- Функция `register_handlers(bot)` регистрирует обработчики:
  - `cmd_start(message)` — обрабатывает команду `/start`, задаёт вопрос о возрасте и при утвердительном ответе выводит системное сообщение.
