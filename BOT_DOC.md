...
- `/start` — спрашивает, есть ли пользователю 18 лет, если его нет в базе. При положительном ответе выводится системное сообщение с правилами, иначе доступ блокируется. Зарегистрированным пользователям бот сразу отвечает `[SYSTEM] ДОБРО ПОЖАЛОВАТЬ`.
