...
- `/end` — завершает текущую сессию, создаёт краткий конспект переписки и
  сообщает дату и время завершения.
...
При закрытии по таймауту бот также сообщает дату и время завершения.
