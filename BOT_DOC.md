...
Проект содержит три независимых телеграм‑бота, расположенных в каталогах `bots/bot1`, `bots/bot2` и `bots/bot3`. Общий код вынесен в пакет `shared`.

## Запуск

Бота можно запустить скриптом `run.py`. Например, `python run.py bot1` запустит первого бота. Вместо `bot1` укажите `bot2` или `bot3`.
